...
class AppController {
  constructor() {
    ...
    this._API_KEY = 'AIzaSyDJ0bql2lkFrvjSlIIXHHLf1bYYHtv5o08';

    this._applicationKeys = {
      publicKey: window.base64UrlToUint8Array(
        'BDd3_hVL9fZi9Ybo2UUzA284WG5FZR30_95YeZJsiApwXKpNcF1rRPF3foIiBHXRdJI2Qh' +
        'umhf6_LFTeZaNndIo'),
    ...
    this._sendPushOptions = document.querySelector(
      '.js-send-push-options');
    this._payloadTextField = document.querySelector(
      '.js-payload-textfield');
  ...
  _uiInitialised(toggleSwitch) {
    ...
    this._pushClient = new PushClient(this._stateChangeListener,
    ...
    document.querySelector('.js-push-toggle-switch > input')
      .addEventListener(
        'click', event => {
          // Inverted because clicking will change the checked state by the time
          // we get here
    ...
    const sendPushViaXHRButton = document.querySelector(
      '.js-send-push-button');
    sendPushViaXHRButton.addEventListener('click', () => {
      if (this._currentSubscription) {
        this.sendPushMessage(this._currentSubscription, this._payloadTextField
          .value);
      }
    });
  ...
  _subscriptionUpdate(subscription) {
    ...
    if (!subscription) {
      // Remove any subscription from your servers if you have set it up.
    ...
    const subscriptionObject = JSON.parse(JSON.stringify(
      subscription));
    if (subscriptionObject && subscriptionObject.keys &&
      subscriptionObject.keys
      .auth && subscriptionObject.keys.p256dh) {
  ...
  updatePushInfo() {
    ...
    if (payloadText && payloadText.trim()
      ...
      payloadPromise = EncryptionHelperFactory.generateHelper()
        .then(
          encryptionHelper => {
            return encryptionHelper.encryptMessage(JSON.parse(JSON.stringify(
    ...
    return Promise.all([payloadPromise, vapidPromise])
      .then(results => {
        ...
        infoFunction = () => {
          return this.getWebPushInfo(this._currentSubscription,
            payload,
            vapidHeaders);
        };
        ...
        let curlCommand =
          `curl "${requestInfo.endpoint}" --request POST`;
        let curlError = null;

        document.querySelector('.js-endpoint')
          .textContent =
          requestInfo.endpoint;
        const headersList = document.querySelector(
          '.js-headers-list');
        ...
        Object.keys(requestInfo.headers)
          .forEach(header => {
            ...
            curlCommand +=
              ` --header "${header}: ${requestInfo.headers[header]}"`;
          });

        const bodyFormat = document.querySelector(
          '.js-body-format');
        ...
        if (requestInfo.body && requestInfo.body instanceof ArrayBuffer) {
          ...
          curlCommand = null;
          curlError =
            'Sorry, but because the web push protocol requires a stream as ' +
          ...
          curlCommand +=
            ` -d ${JSON.stringify(requestInfo.body)}`;
        } else {
        ...
        const curlCodeElement = document.querySelector(
          '.js-curl-code');
        const curlMsgElement = document.querySelector(
          '.js-curl-copy-msg');
  ...
  getGCMInfo(subscription, payload, apiKey) {
    ...
    const subscriptionId = endpointSections[endpointSections.length -
      1];
    const gcmAPIData = {
      to: subscriptionId
    };
  ...
  sendPushMessage(subscription, payloadText) {
    ...
    return Promise.all([payloadPromise, vapidPromise])
      .then(results => {
        ...
        // Some push services don't allow CORS so have to forward it to a
        // different server to make the request which does support CORs
  ...
  sendRequestToProxyServer(requestInfo) {
    ...
    // Can't send a stream like is needed for web push protocol, so needs
    // to convert it to base 64 here and the server will convert back and
    // pass as a stream
    if (requestInfo.body && requestInfo.body instanceof ArrayBuffer) {
    ...
    fetch('https://simple-push-demo.appspot.com/api/v2/sendpush',
        fetchOptions)
  ...
  toBase64(arrayBuffer, start, end) {
    ...
    const partialBuffer = new Uint8Array(arrayBuffer.slice(start,
      end));
  ...
  showErrorMessage(title, message) {
    ...
    const titleElement = errorContainer.querySelector(
      '.js-error-title');
